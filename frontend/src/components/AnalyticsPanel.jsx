...
function StatusDot({ ok }) {
  return (
    <span
      className={`inline-block w-2 h-2 rounded-full ${ok ? "bg-white" : "bg-white/20"}`}
    />
  );
}
...
export default function AnalyticsPanel({
  ...
  const leaderboard = Array.isArray(metrics?.leaderboard)
    ? metrics.leaderboard
    : [];
  const reentryCount = safeEvents.filter((e) => e.event_type === "REENTRY").length;

  const journey = safeEvents
    .filter((e) => e.visitor_id && e.visitor_id !== "SYSTEM")
    .slice(0, 8)
    .map((e) => e.zone_id || e.event_type)
    .filter(Boolean);
  const journeyPath = journey.length
    ? ["ENTRY", ...new Set(journey)]
    : ["ENTRY", "-", "EXIT"];

  const heatData = Array.isArray(heatmapCells) ? heatmapCells : [];
      ...
      {open && (
        <>
          ...
          <motion.aside
            ...
            className="fixed top-0 right-0 h-full w-full max-w-lg z-50 overflow-y-auto border-l border-white/15 bg-[#101113]/95 p-6 text-white shadow-glow backdrop-blur-2xl"
          >
            <header className="flex justify-between items-center mb-6 pb-4 border-b border-white/10">
              <div>
                ...
                <p className="text-xs text-white/40">
                  Live intelligence control center
                </p>
              </div>
              <button
                type="button"
            ...
            <Section title="Live Metrics">
              <div className="grid grid-cols-3 gap-2">
                <MetricTile
                  label="Visitors"
                  value={safeMetrics.active_visitors ?? 0}
                />
                <MetricTile label="Queue" value={safeMetrics.queue_depth ?? 0} />
                <MetricTile
                  label="Conversion"
                  value={((safeMetrics.conversion_rate ?? 0) * 100).toFixed(0)}
                  unit="%"
                />
                <MetricTile label="Anomalies" value={safeAnomalies.length} />
            ...
            <Section title="Anomaly Feed">
              <ul className="space-y-2 max-h-36 overflow-y-auto text-xs font-mono">
                {safeAnomalies.length === 0 && (
                  <li className="text-white/30">No anomalies detected</li>
                )}
                {safeAnomalies.map((a, i) => (
                    ...
                    >
                      ...
                      <span>
                        {a.metadata?.anomaly_type ||
                          a.metadata?.movement_pattern ||
                          a.event_type}
                        {a.zone_id && (
                          <span className="text-white/40"> / {a.zone_id}</span>
                        )}
                      </span>
                    </li>
                  ))}
              ...
              <ul className="space-y-1 max-h-40 overflow-y-auto text-xs font-mono">
                ...
                {safeEvents.slice(0, 25).map((e, i) => (
                    <li key={e.event_id || i} className="truncate">
                      ...
                      {e.zone_id && (
                        <span className="text-white/30"> @ {e.zone_id}</span>
                      )}
                    </li>
                  ))}
              </ul>
            ...
            <Section title="System Health">
              <div className="space-y-2 text-xs">
                <div className="flex items-center gap-2">
                  <StatusDot ok={wsConnected} /> WebSocket{" "}
                ...
                {health?.infrastructure && (
                  <>
                    <div className="flex items-center gap-2">
                      <StatusDot ok={health.infrastructure.kafka?.ok} /> Kafka
                    </div>
                    <div className="flex items-center gap-2">
                      <StatusDot ok={health.infrastructure.redis?.ok} /> Redis
                      Pub/Sub
                    </div>
                    <div className="flex items-center gap-2">
                      <StatusDot ok={health.infrastructure.postgresql?.ok} />{" "}
                      PostgreSQL
                    </div>
                    <div className="flex items-center gap-2">
                      <StatusDot ok={health.infrastructure.websocket?.ok} />{" "}
                      Detection + Gateway
                    </div>
                  </>
                )}
